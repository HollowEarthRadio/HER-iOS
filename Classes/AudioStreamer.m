				...
				for (int i=streamStart; i < length; i++)
				{
					...
					if (myData->metaDataBytesRemaining > 0)
					{
						...
						if (myData->metaDataBytesRemaining == 0)
						{
							...
							[myData displayTitles:myData.metaDataString];
...
-(void)displayTitles:(NSString*)_title {
	...
	//suffix ';StreamUrl=
	if (_title != nil) {
		NSArray *prefix = [_title componentsSeparatedByString: @"StreamTitle='"];
		NSString *prefixString = (NSString*)[prefix objectAtIndex:1 ];
		NSArray *suffix = [prefixString componentsSeparatedByString: @"';StreamUrl=" ];

		NSString *parsed = [suffix objectAtIndex:0];
		[self.uiController.titleLabel setFont:[UIFont fontWithName:@"Helvetica" size:14]];
		[self.uiController.titleLabel setText:parsed];
